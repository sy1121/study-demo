...
public class LeafProgressBar extends View {
    ...
    // 不同振幅差
    private static final int AMPLITUDE_DISPLARITY = 5;
    ...
    // 振幅差
    private int mAmplitudeDisparity = AMPLITUDE_DISPLARITY;
    ...
    private RectF mWhiteRectF,mOrangeWhiteRectF,mArcRectF;
    // 当前进度
    ...
    public LeafProgressBar(Context context, AttributeSet attrs){
        ...
        mLeafinfo =mLeafFactory.generateLeafs();
    }
    ...
    private void initBitmap(){
        mLeafBitmap = ((BitmapDrawable)mResource.getDrawable(R.mipmap.leaf)).getBitmap();
        mLeafWidth = mLeafBitmap.getWidth();
        mLeafHeight = mLeafBitmap.getHeight();

        mOuterBitmap =((BitmapDrawable)mResource.getDrawable(R.mipmap.leaf_kuang)).getBitmap();
    ...
    protected void onSizeChanged(int w, int h, int oldw, int oldh) {
        ...
        mOrangeWhiteRectF =new RectF(mLeftMargin+mArcRadius,mLeftMargin,mCurrentProgressPosition,mTotalHeight-mLeftMargin);
    ...
    private void drawProgressAndLeafs(Canvas canvas){
        ...
        if(mCurrentProgressPosition < mArcRadius){
            ...
            // 3.绘制orange RECT
            mOrangeWhiteRectF.left = mArcRightLocation;
            mOrangeWhiteRectF.right = mCurrentProgressPosition;
            canvas.drawRect(mOrangeWhiteRectF,mOrangePaint);
        }
    }
    ...
    private void drawLeafs(Canvas canvas){
        mLeafRotateTime = mLeafFloatTime<=0?LEAF_FLOAT_TIME:mLeafFloatTime;
        ...
        for(int i=0;i<mLeafinfo.size();i++){
            ...
            if(currentTime > leaf.startTime && leaf.startTime!= 0){
                ...
                float tranX = mLeftMargin + leaf.x;
                float tranY = mLeftMargin + leaf.y;
                matrix.postTranslate(tranX,tranY);
                // 通过时间关联旋转角度，则可以直接修改LEAF_TORATE_TIME调节叶子旋转快慢
                ...
                matrix .postRotate(rotate,tranX+mLeafWidth/2,tranY+ mLeafHeight/2);
    ...
    private void getLeafLocation(Leaf leaf,long currentTime){
        ...
        mLeafFloatTime = mLeafFloatTime <= 0 ? LEAF_ROTATE_TIME:mLeafFloatTime;
        if(intervalTime < 0){
            return ;
    ...
    private class LeafFactory{
        ...
        //根据最大叶子数产生叶子信息
        public List<Leaf> generateLeafs(){ return generateLeafs(MAX_LEAFS);}
